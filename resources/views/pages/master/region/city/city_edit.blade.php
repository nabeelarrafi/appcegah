...
@section('nav')
    ...
    <li class="breadcrumb-item"><a href="{{ route('Admin:Dashboard:Master:City:Index') }}">Kabupaten / Kota</a></li>
            ...
            <div class="col-md-5 col-xl-5 col-xs-12 col-sm-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="mg-b-20 text-muted">Edit kabupaten / kota</p>
                        <form action="{{ route('Admin:Dashboard:Master:City:Update', ['city' => $data['edit']->id_kabupatenkota]) }}" method="post">
                            @method('put')
                            @csrf
                            ...
                            <div class="form-group">
                                <label class="main-content-label tx-11 tx-medium tx-gray-900">Province</label>
                                <select class="form-control" required name="id_province">
                                    <option label="Choose one" value="">
                                    </option>
                                    @foreach ($data['province'] as $province)
                                        <option value="{{ $province->id_provinsi }}" @if($data['edit']->id_provinsi == $province->id_provinsi) selected @endif>
                                            {{ $province->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-footer">
                                ...
                                <a href="{{ route('Admin:Dashboard:Master:City:Index') }}" class="btn btn-outline-danger float-right mr-2">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-md-7 col-xl-7 col-xs-12 col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 pd-t-25">
                        ...
                        <p class="tx-12 text-muted mb-0">Kabupaten / Kota</p>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-md-nowrap table-striped" id="example2">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $city->provinsi }}</td>
                                        <td>
                                            <a href="{{ route('Admin:Dashboard:Master:City:Edit', ['city' => $city->id_kabupatenkota]) }}" class="btn btn-info btn-icon" style="display: inline-block"><i class="typcn typcn-edit"></i></a>
                                            <form data-form-id="{{ $city->id_kabupatenkota }}" action="{{ route('Admin:Dashboard:Master:City:Destroy', ['city' => $city->id_kabupatenkota]) }}" method="POST"  style="display: inline-block">
                                                @csrf
                                                @method('delete')
                                                <button data-btn-id="{{ $city->id_kabupatenkota }}" onclick="confirmDelete(this)" class="btn btn-danger btn-icon" type="button"><i class="typcn typcn-trash"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach

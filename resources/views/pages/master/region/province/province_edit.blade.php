...
@section('nav')
    ...
    <li class="breadcrumb-item"><a href="{{ route('Admin:Dashboard:Master:Province:Index') }}">Provinsi</a></li>
            ...
            <div class="col-md-5 col-xl-5 col-xs-12 col-sm-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="mg-b-20 text-muted">Edit provinsi</p>
                        <form action="{{ route('Admin:Dashboard:Master:Province:Update', ['province' => $data['edit']->id_provinsi]) }}" method="post">
                            @csrf
                            @method('put')
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" required name="id_country">
                                    <option label="Choose one" value="">
                                    </option>
                                    @foreach ($data['country'] as $country)
                                        <option value="{{ $country->id_negara }}" @if($data['edit']->id_negara == $country->id_negara) selected @endif>
                                            {{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-footer">
                                ...
                                <a href="{{ route('Admin:Dashboard:Master:Province:Index') }}" class="btn btn-outline-danger float-right mr-2">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-md-7 col-xl-7 col-xs-12 col-sm-12">
                <div class="card">
                    <div class="card-header pb-0 pd-t-25">
                        ...
                        <p class="tx-12 text-muted mb-0">Provinsi</p>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-md-nowrap table-striped" id="example2">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $province->name }}</td>
                                        <td>{{ $province->negara }}</td>
                                        <td>
                                            <a href="{{ route('Admin:Dashboard:Master:Province:Edit', ['province' => $province->id_provinsi]) }}" class="btn btn-info btn-icon" style="display: inline-block"><i class="typcn typcn-edit"></i></a>
                                            <form data-form-id="{{ $province->id_provinsi }}" action="{{ route('Admin:Dashboard:Master:Province:Destroy', ['province' => $province->id_provinsi]) }}" method="POST"  style="display: inline-block">
                                                @csrf
                                                @method('delete')
                                                <button data-btn-id="{{ $province->id_provinsi }}" onclick="confirmDelete(this)" class="btn btn-danger btn-icon" type="button"><i class="typcn typcn-trash"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
    ...
    @section('script')
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                toggleMenu('.fa-map-marker-alt', 'area/province');
            })
        </script>

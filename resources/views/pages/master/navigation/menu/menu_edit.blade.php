...
<li class="breadcrumb-item"><a href="{{ route('Admin:Dashboard:Master:Menu:Index') }}">Menu</a></li>
            ...
            <div class="col-md-5 col-xl-5 col-xs-12 col-sm-12">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="mg-b-20 text-muted">Edit menu</p>
                        <form action="{{ route('Admin:Dashboard:Master:Menu:Update', ['menu' => $data['edit']->id_menu]) }}" method="post">
                            @method('put')
                            @csrf
                            ...
                            <div class="form-group">
                                <label class="main-content-label tx-11 tx-medium tx-gray-900">Menu Category</label>
                                <select class="form-control" required name="id_menu_category">
                                    <option label="Choose one" value="">
                                    </option>
                                    @foreach ($data['menu_cat'] as $menu_cat)
                                        <option value="{{ $menu_cat->id_menu_category }}" @if($data['edit']->id_menu_category == $menu_cat->id_menu_category) selected @endif>
                                            {{ $menu_cat->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label class="main-content-label tx-11 tx-medium tx-gray-900">Icon Class</label>
                                <input class="form-control" placeholder="Icon Class..." name="fa_class" required type="text" value="{{ $data['edit']->fa_class }}">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-footer">
                                ...
                                <a href="{{ route('Admin:Dashboard:Master:Menu:Index') }}" class="btn btn-outline-danger float-right mr-2">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header pb-0 pd-t-25">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-0">Daftar Menu</h4>
                        </div>
                        <p class="tx-12 text-muted mb-0">Menu</p>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table text-md-nowrap table-striped" id="example2">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $menu->url }}</td>
                                        <td>
                                            <a href="{{ route('Admin:Dashboard:Master:Menu:Edit', ['menu' => $menu->id_menu]) }}" class="btn btn-info btn-icon" style="display: inline-block"><i class="typcn typcn-edit"></i></a>
                                            <form data-form-id="{{ $menu->id_menu }}" action="{{ route('Admin:Dashboard:Master:Menu:Destroy', ['menu' => $menu->id_menu]) }}" method="POST"  style="display: inline-block">
                                                @csrf
                                                @method('delete')
                                                <button data-btn-id="{{ $menu->id_menu }}" onclick="confirmDelete(this)" class="btn btn-danger btn-icon" type="button"><i class="typcn typcn-trash"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
    ...
    @section('script')
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                toggleMenu('.fa-location-arrow', 'navigation/menu');
            })
        </script>

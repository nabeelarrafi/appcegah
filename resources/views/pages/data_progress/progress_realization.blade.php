                                        ...
                                        <table class="table text-md-nowrap table-striped" id="example2">
                                            ...
                                            <tbody>
                                                @foreach ($data['salur_lapor'] as $salur_lapor)
                                                    <tr>
                                                        <td><a href="{{ route('Admin:Dashboard:Realization:Province:Detail', ['kode_wilayah' => $salur_lapor['kode_prop']]) }}">{{ $loop->iteration }}</a></td>
                                                        <td><a href="{{ route('Admin:Dashboard:Realization:Province:Detail', ['kode_wilayah' => $salur_lapor['kode_prop']]) }}">{{ $salur_lapor['prop'] }}</a></td>
                                                        <td><a href="{{ route('Admin:Dashboard:Realization:Province:Detail', ['kode_wilayah' => $salur_lapor['kode_prop']]) }}">{{ $salur_lapor['tahap'] }}</a></td>
                                                        <td><a href="{{ route('Admin:Dashboard:Realization:Province:Detail', ['kode_wilayah' => $salur_lapor['kode_prop']]) }}">Rp. {{ number_format($salur_lapor['nilai_salur']) }}</a></td>
                                                        <td><a href="{{ route('Admin:Dashboard:Realization:Province:Detail', ['kode_wilayah' => $salur_lapor['kode_prop']]) }}">Rp. {{ number_format($salur_lapor['nilai_penggunaan']) }}</a></td>
                                                        <td><a href="{{ route('Admin:Dashboard:Realization:Province:Detail', ['kode_wilayah' => $salur_lapor['kode_prop']]) }}">{{ round(($salur_lapor['nilai_penggunaan']/$salur_lapor['nilai_salur'])*100, 2) }}%</a></td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
    ...
    <script>
        $(function() {
            ...
            @if($data['region'] === 'Provinsi' && $data['is_detail'])
                toggleMenu('.fa-dollar-sign', 'realization/provinsi');
            @elseif($data['region'] === 'KabupatenKota' && $data['is_detail'])
            toggleMenu('.fa-dollar-sign', 'realization/kabupatenkota');
            @endif

            @if($data['region'] === 'Provinsi')
                $('.search').click(function() {
                    let id  = $('select[name="provinsi"]').val();
                    let url = `{{ route('Admin:Dashboard:Realization:Province') }}/${id}`;
            ...
            @if($data['region'] === 'KabupatenKota')
                ...
                $('.search').click(function() {
                    ...
                    let url = `{{ route('Admin:Dashboard:Realization:City') }}/${id}`;
